...
(function () {
  ...
  // --- Icônes (trait fin, aucun emoji — charte Keepnew) ----------------------
  ...
  var ICON = {
    home: svgIcon('<path d="M3 11.5 12 4l9 7.5"/><path d="M5.5 10v9a1 1 0 0 0 1 1H9a1 1 0 0 0 1-1v-4h4v4a1 1 0 0 0 1 1h2.5a1 1 0 0 0 1-1v-9"/>'),
    workshop: svgIcon('<path d="M14.7 6.3a4 4 0 0 0-5.4 5.4L3 18l3 3 6.3-6.3a4 4 0 0 0 5.4-5.4l-2.6 2.6-2-2z"/>'),
    category: svgIcon('<rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/>'),
    service: svgIcon('<path d="M12 3s6 6.5 6 11a6 6 0 0 1-12 0c0-4.5 6-11 6-11z"/>'),
  };
  ...
  function renderWhere(body) {
    ...
    cards.appendChild(
      choiceCard(ICON.home, "Je veux qu'on vienne chez moi", "Un technicien se déplace à votre adresse.", function () {
        pickMode("onsite");
      }, state.mode === "onsite")
    );
    cards.appendChild(
      choiceCard(ICON.workshop, "Je viens à l'atelier", "Vous déposez, nous nettoyons. Souvent moins cher.", function () {
        pickMode("workshop");
      }, state.mode === "workshop")
    );
  ...
  function renderWhat(body) {
    ...
    if (!state.categoryId) {
      catalog.categories.forEach(function (cat) {
        cards.appendChild(
          choiceCard(ICON.category, cat.name, cat.description || "", function () {
      ...
      (cat && cat.children && cat.children.length ? cat.children : [cat]).forEach(function (c) {
        catalog.services
          ...
          .forEach(function (svc) {
            cards.appendChild(
              choiceCard(ICON.service, svc.name, svc.short_description || "", function () {
  ...
  function renderDone(body) {
    ...
    body.appendChild(el('<div class="kn-done"><p class="kn-done-script">Merci !</p><div class="kn-done-mark">✓</div>' +
  ...
  function CSS() {
    return (
      "@import url('https://fonts.googleapis.com/css2?family=Bodoni+Moda:ital,opsz,wght@0,6..96,400..700;1,6..96,400..700&family=Jost:ital,wght@0,300..700;1,400&family=Great+Vibes&display=swap');" +
      ".kn{--a:#4A63E7;--ai:#3B50C9;--blush:#EFC6CB;--ink:#1B2A4A;--muted:#6E7891;--paper:#F7F4EC;--surface:#fff;--line:#E4DECF;--line-strong:#C9C2AF;--ok:#3E7C4F;--alert:#B3453E;--fd:'Bodoni Moda',Didot,serif;--fs:'Great Vibes',cursive;" +
      "font-family:'Jost','Century Gothic',sans-serif;font-size:16px;line-height:1.6;color:var(--ink);background:var(--paper);max-width:560px;margin:0 auto;padding:16px;box-sizing:border-box}" +
      ".kn *{box-sizing:border-box}" +
      ".kn-h{font-family:var(--fd);font-weight:600;font-size:1.6rem;margin:8px 0 16px}.kn-h3{font-family:var(--fd);font-weight:600;font-size:1.15rem;margin:16px 0 8px}" +
      ...
      ".kn-pill{background:var(--blush);color:var(--ink);border:0;border-radius:999px;padding:4px 14px;font-size:.75rem;font-weight:500;letter-spacing:.04em;cursor:pointer}" +
      ".kn-steps{display:flex;gap:4px;flex-wrap:wrap}" +
      ".kn-step{font-size:.68rem;font-weight:500;letter-spacing:.08em;text-transform:uppercase;color:var(--muted);padding:2px 6px;border-bottom:2px solid var(--line)}" +
      ".kn-step.on{color:var(--ink);border-color:var(--a)}" +
      ".kn-cards{display:grid;gap:12px;margin:12px 0}.kn-cards-sm{grid-template-columns:repeat(auto-fill,minmax(120px,1fr))}" +
      ".kn-card{display:flex;flex-direction:column;gap:4px;text-align:left;background:var(--surface);border:1px solid var(--line);border-radius:6px;box-shadow:0 1px 2px rgba(27,42,74,.06),0 8px 24px rgba(27,42,74,.06);padding:16px;min-height:48px;cursor:pointer;font:inherit;color:inherit;transition:border-color 120ms cubic-bezier(.4,0,.2,1)}" +
      ".kn-card.on{border-color:var(--a);box-shadow:0 0 0 2px var(--a) inset}" +
      ".kn-card-icon{display:inline-flex;color:var(--a)}.kn-card-title{font-weight:600}.kn-card-desc{color:var(--muted);font-size:.85rem}" +
      ".kn-field{margin:12px 0}.kn-field label{display:block;font-size:.85rem;color:var(--muted);margin-bottom:4px}" +
      ".kn input[type=text],.kn input[type=email],.kn input[type=tel],.kn input:not([type]){width:100%;min-height:48px;padding:0 12px;font-size:16px;font-family:inherit;border:1px solid var(--line);border-radius:3px;background:var(--surface);color:var(--ink)}" +
      ".kn input:focus{outline:2px solid var(--a);outline-offset:2px}" +
      ".kn-extras{display:flex;flex-direction:column;gap:8px}" +
      ".kn-extra{display:flex;justify-content:space-between;align-items:center;gap:12px;border:1px solid var(--line);border-radius:3px;padding:12px;min-height:48px;cursor:pointer}" +
      ".kn-extra input{width:22px;height:22px}" +
      ".kn-choice-row{display:flex;gap:8px;flex-wrap:wrap}" +
      ".kn-chip{min-height:48px;padding:0 20px;border:1px solid var(--line);border-radius:3px;background:var(--surface);color:inherit;font:inherit;cursor:pointer}" +
      ".kn-chip.on{border-color:var(--a);background:var(--a);color:#fff}" +
      ".kn-live{margin:16px 0;text-align:center}.kn-live-price{font-family:var(--fd);font-size:2rem;font-weight:600;font-variant-numeric:tabular-nums}" +
      ".kn-actions{display:flex;flex-direction:column;gap:8px;margin:16px 0}" +
      ".kn-btn{min-height:48px;padding:0 24px;border-radius:3px;font:inherit;font-weight:500;border:1px solid transparent;cursor:pointer;transition:background 120ms cubic-bezier(.4,0,.2,1),color 120ms cubic-bezier(.4,0,.2,1)}" +
      ".kn-btn-primary{background:var(--a);color:#fff}.kn-btn-primary:hover{background:var(--ai)}" +
      ".kn-btn-ghost{background:transparent;color:var(--ai);border-color:var(--line-strong)}" +
      ...
      ".kn-badge{display:inline-block;font-size:.7rem;font-weight:500;letter-spacing:.03em;background:var(--blush);color:var(--ink);border-radius:999px;padding:1px 10px}" +
      ".kn-reassure{background:#E5E9FB;border-left:3px solid var(--a);padding:8px 12px;border-radius:0 3px 3px 0;font-size:.875rem;color:var(--ai)}" +
      ".kn-alert{background:#F7E4E2;color:var(--alert);padding:12px;border-radius:3px}" +
      ...
      ".kn-total{border-bottom:0;border-top:2px solid var(--ink);font-family:var(--fd);font-weight:600;font-size:1.2rem;margin-top:4px}" +
      ".kn-ok{color:var(--ok)}" +
      ".kn-dates{display:flex;gap:8px;overflow-x:auto;padding-bottom:8px}" +
      ".kn-date{white-space:nowrap;min-height:44px;padding:0 12px;border:1px solid var(--line);border-radius:3px;background:var(--surface);font:inherit;cursor:pointer}" +
      ".kn-date.on{border-color:var(--a);color:var(--a);font-weight:600}" +
      ".kn-slot-list{display:grid;gap:8px;margin-top:8px}" +
      ".kn-slot{min-height:48px;border:1px solid var(--line);border-radius:3px;background:var(--surface);font:inherit;cursor:pointer}" +
      ".kn-slot.on{border-color:var(--a);background:var(--a);color:#fff}" +
      ".kn-quotebar{position:sticky;bottom:0;width:100%;display:flex;justify-content:space-between;align-items:center;min-height:56px;padding:0 16px;margin-top:16px;background:var(--ink);color:#fff;border:0;border-radius:6px;font:inherit;cursor:pointer}" +
      ".kn-quotebar-total{font-family:var(--fd);font-weight:600;font-size:1.15rem;font-variant-numeric:tabular-nums}" +
      ".kn-flash{position:fixed;left:50%;bottom:24px;transform:translateX(-50%);background:var(--ink);color:#fff;padding:12px 20px;border-radius:3px;z-index:9999}" +
      ".kn-done{text-align:center;padding:24px 0}" +
      ".kn-done-script{font-family:var(--fs);font-size:2.4rem;color:#D08E99;margin:0 0 4px;line-height:1}" +
      ".kn-done-mark{width:64px;height:64px;line-height:64px;border-radius:50%;background:var(--ok);color:#fff;font-size:2rem;margin:0 auto 16px}" +
